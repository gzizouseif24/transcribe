...
const App: React.FC = () => {
  ...
  if (isCheckingKey) return <div className="min-h-screen bg-slate-50 flex items-center justify-center">Loading...</div>;
  if (!hasApiKey) return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-lg p-8 text-center max-w-md">
           <h1 className="text-xl font-bold mb-4">API Key Required</h1>
           <button onClick={handleSelectKey} className="bg-indigo-600 text-white px-4 py-2 rounded">Select Key</button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      <header className="bg-white border-b border-slate-200 h-16 flex items-center px-4 sticky top-0 z-10 shadow-sm">
        <h1 className="text-xl font-bold text-indigo-700">Tunisian Transcriber: QA Workflow</h1>
      </header>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-wrap gap-4 items-center justify-between">
           ...
           <div className="flex gap-3">
              <button 
                ...
                className="px-4 py-2 bg-slate-800 text-white rounded-lg text-sm font-medium hover:bg-slate-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
